...
function Payslip({ payrollRun, employeeData, employeeProfile }) {
  ...
  const grossPay = (employeeData.baseSalary || 0) + (employeeData.bonuses || 0); // Simplified
  ...
  const handleDownloadPdf = () => {
    ...
    // Header
    doc.setFontSize(20);
    doc.text(companyInfo?.companyName || 'Company Name', 15, 20);
    doc.setFontSize(10);
    doc.text(companyInfo?.companyAddress || 'Company Address', 15, 26);
    ...
    const deductions = [
        ['CNSS & AMO', ((employeeData.cnss || 0) + (employeeData.amo || 0)).toFixed(2)],
    ...
    doc.autoTable({
        ...
        head: [['Gains', 'Montant (MAD)']], body: earnings, theme: 'striped', headStyles: { fillColor: [22, 160, 133] }
    });
    doc.autoTable({
        startY: doc.lastAutoTable.finalY + 2,
        head: [['Retenues', 'Montant (MAD)']], body: deductions, theme: 'striped', headStyles: { fillColor: [231, 76, 60] }
    });
  ...
  return (
    <div className="bg-white p-8 rounded-lg shadow-lg border max-w-4xl mx-auto font-sans">
        <div className="flex justify-between items-start mb-4">
            <div>
                <h3 className="text-2xl font-bold text-gray-800">{companyInfo?.companyName || 'Company Name'}</h3>
                <p className="text-sm text-gray-500">{companyInfo?.companyAddress || 'Company Address'}</p>
            </div>
            <button onClick={handleDownloadPdf} className="flex items-center gap-2 bg-blue-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-blue-700"><Download size={16}/> Download PDF</button>
        </div>
        <hr className="my-4"/>
        <h2 className="text-center font-bold text-xl my-4">BULLETIN DE PAIE - {payrollRun.periodLabel}</h2>
        <div className="text-right mt-8"><p className="font-bold text-lg">Net à Payer: {netPay.toFixed(2)} MAD</p></div>
    </div>
  );
}
