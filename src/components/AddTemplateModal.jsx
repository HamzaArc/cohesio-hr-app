...
function AddTemplateModal({ isOpen, onClose, onTemplateAdded }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await addDoc(collection(db, 'reviewTemplates'), {
        name,
        description,
      ...
      <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-lg">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mt-8 flex justify-end">
            ...
            <button type="submit" disabled={loading} className="bg-blue-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-blue-700 disabled:bg-gray-400">{loading ? 'Saving...' : 'Save Template'}</button>
          </div>
        </form>
      </div>
    </div>
