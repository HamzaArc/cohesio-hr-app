...
function AddDocumentModal({ isOpen, onClose, onDocumentAdded }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!name || !file || !companyId) {
      setFormError('Please provide a name and a document.');
      return;
    }
    setFormError('');

    try {
      const fileURL = await uploadFile(file, `documents/${companyId}`);
  ...
  const handleClose = () => {
    setName(''); setFile(null); setExpirationDate(''); setCategory('Policy');
    setDescription(''); setAssignedEmails([]); setAssignmentType('all');
    setFormError(''); onClose();
  };
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl flex flex-col max-h-full">
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto p-6">
          <div className="space-y-4">
            ...
            <div><label htmlFor="file" className="block text-sm font-medium text-gray-700">File</label><input type="file" id="file" onChange={(e) => setFile(e.target.files[0])} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2" /></div>
              ...
              <div className="border rounded-lg p-2 max-h-48 overflow-y-auto">
                {employees.map(emp => (
                  <div key={emp.id} className="flex items-center p-2 rounded-md hover:bg-gray-50">
                    <input type="checkbox" id={`emp-${emp.id}`} checked={assignedEmails.includes(emp.email)} onChange={() => handleEmployeeToggle(emp.email)} className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500" />
                    <label htmlFor={`emp-${emp.id}`} className="ml-3 text-sm text-gray-700">{emp.name}</label>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="mt-8 pt-6 border-t flex justify-end">
            ...
            <button type="submit" disabled={uploading} className="bg-blue-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-blue-700 disabled:bg-gray-400">{uploading ? 'Uploading...' : 'Save Document'}</button>
          </div>
        </form>
      </div>
    </div>
