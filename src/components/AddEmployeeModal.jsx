...
function AddEmployeeModal({ isOpen, onClose, onEmployeeAdded }) {
  ...
  const [formData, setFormData] = useState({
    ...
    employmentType: 'Full-time', compensation: '', managerEmail: '', contractType: 'CDI',
    contractEndDate: '', weeklyHours: '', workMode: 'on-site', monthlyGrossSalary: '', hourlyRate: '',
    // Personal & Contact
  ...
  const handleClose = () => {
    setFormData({ name: '', email: '', position: '', department: '', hireDate: '', status: 'active', employmentType: 'Full-time', compensation: '', managerEmail: '', contractType: 'CDI', contractEndDate: '', weeklyHours: '', workMode: 'on-site', monthlyGrossSalary: '', hourlyRate: '', phone: '', address: '', dateOfBirth: '', nationality: '', maritalStatus: 'Single', personalEmail: '', emergencyContactName: '', emergencyContactRelationship: '', emergencyContactPhone: '', nationalId: '', cnieExpiryDate: '', cnssNumber: '', cnssEnrollmentDate: '', amoScheme: '', cimrEnrollment: false, cimrRate: '', rib: '', bankBranch: '', vacationBalance: 15, sickBalance: 5, personalBalance: 3 });
    setErrors({});
    setActiveTab('job');
    onClose();
  };
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto p-6" noValidate>
          ...
          <div className="mt-8 pt-6 border-t flex justify-end">
            ...
            <button type="submit" disabled={loading || !isFormValid} className="bg-blue-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed">{loading ? 'Saving...' : 'Save Employee'}</button>
          </div>
        </form>
      </div>
    </div>
