...
import { Edit2, Trash2, Plus, DollarSign, ArrowUp, Briefcase, User, Users, Home, Phone, Shield, Plane, Calendar, Heart, Clock, Sun, FileText, Flag, Mail, Lock, Building, List } from 'lucide-react';
...
function EmployeeProfile() {
  ...
  useEffect(() => {
    ...
    return () => { unsubscribeEmployee(); unsubscribeJourney(); };
  ...
  return (
    <>
      ...
      <div className="p-8">
        <div className="flex justify-between items-center mb-6"><Link to="/people" className="text-sm text-blue-600 font-semibold inline-block hover:underline">&larr; Back to People</Link><button onClick={() => setIsDeleteModalOpen(true)} className="flex items-center text-sm text-red-600 bg-red-100 hover:bg-red-200 font-semibold py-2 px-4 rounded-lg"><Trash2 size={16} className="mr-2" />Delete Employee</button></div>
            ...
            <div className="mt-6">
              {activeTab === 'Job' && (
                <div className="space-y-6">
                  ...
                  <InfoSection title="Compensation" onEdit={() => setIsEditModalOpen(true)}><InfoField icon={<DollarSign size={16} />} label="Monthly Gross Salary" value={employee.monthlyGrossSalary ? `${employee.monthlyGrossSalary} MAD` : 'N/A'} /><InfoField icon={<DollarSign size={16} />} label="Hourly Rate" value={employee.hourlyRate ? `${employee.hourlyRate} MAD` : 'N/A'} /><InfoField icon={<DollarSign size={16} />} label="Legacy Compensation" value={employee.compensation} /></InfoSection>
                  <InfoSection title="IT & Equipment" onEdit={() => setIsEditModalOpen(true)}><InfoField icon={<List size={16} />} label="Roles / Applications" value={employee.rolesAuthorizedApplications} /><InfoField icon={<Briefcase size={16} />} label="Equipment" value={employee.equipment} /><InfoField icon={<User size={16} />} label="Badge Number" value={employee.badgeTypeNumber} /></InfoSection>
                </div>
              )}
              {activeTab === 'Personal' && (
                <div className="space-y-6">
